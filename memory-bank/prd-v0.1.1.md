...
- **Padding guidelines:** 120 px safe area left/right/top for all networks; bottom tag bar sits inside its own 40 px bottom gutter.
- **Typography:** (Using 'Inter' font family)
    - Team name = 20 px bold, top-left.
    - Headline = 96 px (desktop/export) / 72 px (mobile preview), weight 700.
    - “Looking for” = 48 px.
    - Bottom Tag (`ai-hackathon.co`) = 16 px absolute, weight 600.
...
6.  Draw red `ai-hackathon.co` text at (x = 120 px, y = `tagY`, color='#FF2A2A', font=16px, weight 600 Inter, align left, baseline middle).

## 4 · QA Checklist Additions

- [ ] ✅ Bottom tag (`ai-hackathon.co`) visible in every export size, never cropped.
...
- Update `CardCanvas` constants (`BOTTOM_TAG_TEXT`, `BOTTOM_TAG_HEIGHT`) and drawing logic for:
    - Team name position (top-left).
    - Bottom tag text (`ai-hackathon.co`) rendering.
